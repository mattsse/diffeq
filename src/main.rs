...
use na::{DefaultAllocator, DimName, MatrixMN, RealField, Unit, Vector2, Vector3, VectorN, U1, U2};
...
pub enum Step<N: RealField, D: Dim>
where
    DefaultAllocator: Allocator<N, U1, D> + Allocator<N, U2, D>,
{
    Fixed(MatrixMN<N, U1, D>),
    Adaptive(MatrixMN<N, U2, D>),
}

pub struct Demo<N: RealField, D: Dim>
where
    N: RealField,
    D: Dim,
